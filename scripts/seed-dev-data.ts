...
async function main() {
  ...
  for (const station of createdStations) {
    try {
      const hashedPassword = await bcrypt.hash('RadioUser123!', 12);
  ...
  const authors = await prisma.user.findMany({
    where: { 
      ...
      staffRole: { in: [StaffRole.JOURNALIST, StaffRole.SUB_EDITOR, StaffRole.EDITOR] }
    }
  });
    ...
    try {
      ...
      const story = await prisma.story.create({
        data: {
          ...
          language: template.tags.includes('Afrikaans') ? StoryLanguage.AFRIKAANS : StoryLanguage.ENGLISH,
          authorId: author.id,
