...
import {
  ...
  useUploadPodcastEpisodeAudio,
  useDeletePodcastEpisodeAudio,
  useLinkAudioToPodcastEpisode,
  usePublishPodcastEpisode,
  useUnpublishPodcastEpisode
} from '@/hooks/use-podcast-episodes';
import { AudioPickerModal } from '@/components/newsroom/AudioPickerModal';
import { canManagePodcasts, canPublishPodcastEpisode } from '@/lib/permissions';
...
export default function PodcastEpisodeDetailPage({
  ...
  const { id: podcastId, episodeId } = use(params);
  ...
  const { data: episode, isLoading } = usePodcastEpisode(podcastId, episodeId);
  const updateEpisode = useUpdatePodcastEpisode();
  const deleteEpisode = useDeletePodcastEpisode();
  const uploadAudio = useUploadPodcastEpisodeAudio();
  const deleteAudio = useDeletePodcastEpisodeAudio();
  const linkAudio = useLinkAudioToPodcastEpisode(podcastId, episodeId);
  const publishEpisode = usePublishPodcastEpisode();
  const unpublishEpisode = useUnpublishPodcastEpisode();
  ...
  const canPublish = canPublishPodcastEpisode(userRole as any, isProducer);

  const handleSave = async (data: EpisodeFormData) => {
    try {
      await updateEpisode.mutateAsync({ podcastId, episodeId, data });
  ...
  const handleDelete = async () => {
    try {
      await deleteEpisode.mutateAsync({ podcastId, episodeId });
      toast.success('Episode deleted successfully');
      router.push(`/newsroom/podcasts/${podcastId}`);
    } catch (error: any) {
  ...
  const handleUploadStaged = async () => {
    ...
    try {
      ...
      await uploadAudio.mutateAsync({ podcastId, episodeId, files });
  ...
  const handleAudioDeleteConfirm = async () => {
    ...
    try {
      await deleteAudio.mutateAsync({ podcastId, episodeId, audioClipId: deleteAudioId });
  ...
  const handleUnpublishConfirm = async () => {
    try {
      await unpublishEpisode.mutateAsync({ podcastId, episodeId });
        ...
        <PageHeader
          ...
          actions={
            <div className="flex gap-2">
              <Button color="white" onClick={() => router.push(`/newsroom/podcasts/${podcastId}`)}>
