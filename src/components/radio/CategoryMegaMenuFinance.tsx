// Finance Hierarchical Menu Component - extracted for clarity
...
export function FinanceHierarchicalMenu({ onClose }: { onClose: () => void }) {
  const [selectedCategory, setSelectedCategory] = useState<string>('Bizskoops');

  // Fetch level 2 categories for Finance
  const { data: level2Data, isLoading: categoriesLoading } = useQuery({
    queryKey: ['finance-level2'],
    queryFn: async () => {
      const response = await fetch('/api/radio/finance-categories');
      if (!response.ok) throw new Error('Failed to fetch finance categories');
  ...
  const { data: storiesData, isLoading: storiesLoading } = useQuery({
    queryKey: ['finance-recent-stories', selectedCategory],
    queryFn: async () => {
